...
public class WebMigrationServiceTests
{
	...
	private Dictionary<string, string> FakeEmptyBucket()
	{
		...
		A.CallTo(() => _s3Client.PutObjectAsync(A<PutObjectRequest>._, A<CancellationToken>._))
			.ReturnsLazily((PutObjectRequest request, CancellationToken _) =>
			{
	...
	private void FakeExistingKeys(IReadOnlyDictionary<string, string> etags) =>
		A.CallTo(() => _s3Client.GetObjectMetadataAsync(A<GetObjectMetadataRequest>._, A<CancellationToken>._))
			.ReturnsLazily((GetObjectMetadataRequest request, CancellationToken _) =>
	...
	public async Task PutFailure_IsReportedPerKeyAndFailsTheRun()
	{
		...
		service.LastResults.Where(r => r.Outcome == "failed" && r.Detail.Contains("Access Denied"))
			.Should().HaveCount(InScopeVersions.Length);
	}
	...
	public async Task VersionsBeyondTheCutoff_AreSkippedAndNeverUploaded()
	{
		...
		A.CallTo(() => _s3Client.PutObjectAsync(
			A<PutObjectRequest>.That.Matches(r => r.Key == Key("2.0.0")), A<CancellationToken>._
		)).MustNotHaveHappened();
	...
	public async Task VersionsFilter_RestrictsTheRunToTheSelection()
	{
		...
		result.Should().BeTrue();
		A.CallTo(() => _s3Client.PutObjectAsync(
			A<PutObjectRequest>.That.Matches(r => r.Key == Key("1.9.0")), A<CancellationToken>._
		)).MustHaveHappenedOnceExactly();
	...
	public async Task FetchesTheMarkdownFromThePinnedRefOnRawGithubusercontent()
	{
		...
		_httpHandler.RequestedPaths.Should().Equal(
			"/elastic/elastic-otel-java/9a61ce4faaf08e272c433a083bcc6f0e96d80e0a/docs/release-notes/index.md");
	}
