...
public class ChangelogPrBodyReaderTests()
{
	[Test]
	public async Task ReadAsync_PrBodyFileUnderWorkingDir_ReadsBody()
	{
		...
		var result = await ChangelogPrBodyReader.ReadAsync(bodyPath, collector, fs, TestContext.Current!.Execution.CancellationToken);
	...
	public async Task ReadAsync_PrBodyFileMissing_EmitsWarning()
	{
		...
		var result = await ChangelogPrBodyReader.ReadAsync(bodyPath, collector, scopedFs, TestContext.Current!.Execution.CancellationToken);
