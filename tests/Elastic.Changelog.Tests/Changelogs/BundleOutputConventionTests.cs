...
public class BundleOutputConventionTests(ITestOutputHelper output) : ChangelogTestBase(output)
{
	...
	private async Task<string> WriteConfig(string configContent)
	{
		...
		await FileSystem.File.WriteAllTextAsync(FileSystem.Path.Join(_changelogDir, "entry.yaml"), Entry, TestContext.Current.CancellationToken);
		...
		await FileSystem.File.WriteAllTextAsync(configPath, configContent.Replace("CHANGELOG_DIR", _changelogDir), TestContext.Current.CancellationToken);
		return configPath;
	}
	...
	public async Task ProfileWithOutputPattern_EmitsHardError()
	{
		...
		Collector.Diagnostics.Should().Contain(d =>
			...
			d.Message.Contains("'output' is no longer supported") &&
	...
	public async Task OutputPatternOnAnotherProfile_AlsoErrors()
	{
		...
		result.Should().BeFalse();
		Collector.Diagnostics.Should().Contain(d =>
			d.Severity == Severity.Error && d.Message.Contains("Profile 'legacy'"));
	}
	...
	public async Task ProfilesCollidingOnPrimaryProduct_EmitError()
	{
		...
		Collector.Diagnostics.Should().Contain(d =>
			d.Severity == Severity.Error &&
			d.Message.Contains("'es-all', 'es-ga'") &&
	...
	public async Task ProfileWithoutOutput_WritesConventionalName()
	{
		...
		FileSystem.File.Exists(FileSystem.Path.Join(_changelogDir, "elasticsearch-9.3.0.yaml"))
			.Should().BeTrue("bundle names derive from the primary output product and version");
	}
