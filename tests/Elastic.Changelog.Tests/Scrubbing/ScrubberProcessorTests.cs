	...
	public ScrubberProcessorTests()
	{
		...
		_ =
			A.CallTo(() => _scrubber.ScrubAsync(A<string>._, A<string>._, A<Cancel>._)).ReturnsLazily(
				(string _, string content, Cancel _) => Task.FromResult("scrubbed: " + content)
			);
		...
		_processor =
			new ScrubberProcessor(NullLoggerFactory.Instance, _s3.Client, PublicBucket, _scrubber, reconciler, shallowReconciler, _metrics);
	}
