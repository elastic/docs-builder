...
public class SiteDocumentationSetsTests(ITestOutputHelper output)
{
	...
	public void SiteNavigationIntegratesWithDocumentationSets()
	{
		...
		var securityDocset = DocumentationSetFile.LoadAndResolve(securityContext.Collector, fileSystem.FileInfo.New("/checkouts/current/serverless-security/docs/_docset.yml"), new CheckoutsFileSystem(fileSystem.DirectoryInfo.New("/checkouts"), inner: fileSystem));
	...
	public void ObservabilityDocumentationSetNavigationHasNoDiagnostics()
	{
		...
		var docset = DocumentationSetFile.LoadAndResolve(context.Collector, docsetPath, new CheckoutsFileSystem(fileSystem.DirectoryInfo.New("/checkouts"), inner: fileSystem));
