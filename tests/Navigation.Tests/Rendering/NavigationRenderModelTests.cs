...
public class NavigationRenderModelTests(ITestOutputHelper output) : DocumentationSetNavigationTestBase(output)
{
	...
	public async Task Create_BuildsBackLinkStack_RootFirst()
	{
		...
		model.RootIndex.Should().BeNull("island index is the Overview leaf, not a detached row");
		model.TreeHeading.Should().Be("index", "TestDocumentationFileFactory uses filename without extension");
		model.TreeHeadingIcon.Should().BeNull();
		model.Tree.Should().HaveCount(2);
		var overview = model.Tree[0];
		overview.Kind.Should().Be(NavigationRenderNodeKind.Leaf);
		overview.NavigationTitle.Should().Be("Overview");
		overview.Url.Should().Be("/security/rules");
	...
	public async Task Create_ContentHash_DiffersByTreeStructure()
	{
		...
		model1.Tree.Should().Contain(n => n.Url.EndsWith("page-a", StringComparison.Ordinal));
	...
	public async Task Create_ThreeLevelIslands_StacksAncestors_RootFirst()
	{
		...
		fileSystem.AddFile(
			"/docs/reference/clients/toc.yml",
			new MockFileData(
				"""
			toc:
			  - file: index.md
		...
		fileSystem.AddFile(
			"/docs/reference/clients/dotnet/toc.yml",
			new MockFileData("""
			toc:
			  - file: index.md
			""")
		...
		var reference = navigation
			...
			.BeOfType<TableOfContentsNavigation<TestDocumentationFile>>()
			.Subject;
		var clients = reference.NavigationItems.ElementAt(0).Should().BeOfType<TableOfContentsNavigation<TestDocumentationFile>>().Subject;
		var dotnet = clients.NavigationItems.ElementAt(0).Should().BeOfType<TableOfContentsNavigation<TestDocumentationFile>>().Subject;
