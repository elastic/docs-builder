...
public class DetectionRulesDocsBuilderExtension(BuildContext build) : IDocsBuilderExtension
{
	...
	public IEnumerable<string> ExternalScopeRoots =>
		GetAllDetectionRuleOverviewRefs(Build.ConfigurationYaml.TableOfContents).SelectMany(r => r.DetectionRuleFolders).Select(
			f => Path.GetFullPath(f, Build.DocumentationSourceDirectory.FullName)
		).Distinct();
