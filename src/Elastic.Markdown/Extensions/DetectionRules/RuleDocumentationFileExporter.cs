	...
	public override async ValueTask ProcessFile(ProcessingFileContext context, Cancel ctx)
	{
		...
		switch (context.File)
		{
			case DetectionRuleFile df:
				context.MarkdownDocument = await htmlWriter.WriteAsync(context.BuildContext.OutputDirectory, DetectionRuleFile.OutputPath(outputFile, context.BuildContext), df, conversionCollector, ctx);
				break;
			case MarkdownFile markdown:
				context.MarkdownDocument = await htmlWriter.WriteAsync(context.BuildContext.OutputDirectory, outputFile, markdown, conversionCollector, ctx);
				break;
			default:
