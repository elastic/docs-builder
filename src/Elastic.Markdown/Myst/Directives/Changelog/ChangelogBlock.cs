...
public class ChangelogBlock(DirectiveBlockParser parser, ParserContext context) : DirectiveBlock(parser, context)
{
	...
	/// <summary>
	/// Whether to render separated types (breaking changes, deprecations, known issues) and
	/// highlight entries as Myst dropdown sections. When false (default), these render as
	/// flattened bulleted lists.
	/// </summary>
	...
	/// <summary>
	/// When true, emits a <c>### Highlights</c> section for entries with <c>highlight: true</c>
	/// (see <c>:highlights:</c> option). Highlighted entries still appear under their type sections.
	/// Defaults to false (inline only).
	/// </summary>
	public bool HighlightsEnabled { get; private set; }
	...
	/// Legacy <c>highlight</c> is rejected; use <c>:highlights:</c> instead.
	/// </summary>
	private ChangelogTypeFilter ParseTypeFilter()
	{
		...
		return typeValue.ToLowerInvariant() switch
		{
			...
			"highlight" => EmitLegacyHighlightTypeWarning(),
