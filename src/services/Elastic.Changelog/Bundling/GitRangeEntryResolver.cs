...
public class GitRangeEntryResolver(IGitHubPrService prService, ILogger logger)
{
	...
	private sealed record ParsedCandidate(
		...
		string? ParseError,
		IReadOnlyList<string> NormalizedPrs
	);
	...
	private static bool MatchesPr(ParsedCandidate candidate, int prNumber, GitRangeEntryResolutionOptions options)
	{
		...
		return candidate.NormalizedPrs.Any(
			pr => ChangelogBundlingService.NormalizePrForComparison(pr, options.Owner, options.Repo) == expected
		);
	}
