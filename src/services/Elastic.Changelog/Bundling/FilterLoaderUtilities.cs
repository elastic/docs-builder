...
internal static class FilterLoaderUtilities
{
	...
	internal static string ExpandTilde(string path)
	{
		...
		return Path.IsPathRooted(relativePath) ? relativePath : Path.Join(homeDirectory, relativePath);
	}
	...
	private static async Task<bool> ReadUrlsFromFileAsync(
		...
		string filePath,
		HashSet<string> valuesToMatch,
		string exampleUrlSegment,
		Cancel ctx
	)
	{
	...
	private static bool ValidateNumericValues(
		...
		string? owner,
		string? repo,
		string numericValidationMessage
	)
	{
		var hasNumericOnly = valuesToMatch.Where(v => !IsUrl(v) && !IsShortFormat(v)).Any(v => int.TryParse(v, out _));
