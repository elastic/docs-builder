...
) : IService
{
	...
	)
	{
		try
		{
			...
			_logger.LogInformation(
				"Detected format: {Format}, found {Count} PR references",
				parsedNotes.Format,
				parsedNotes.PrReferences.Count
			);
	...
	)
	{
		...
		if (
			format == ReleaseNoteFormat.ReleaseDrafter
			&& warnOnTypeMismatch
	...
	)
	{
		...
		var prUrls = createdFileNames.Select(filename =>
		{
