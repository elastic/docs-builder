...
public static class SharedMappingConfig
{
	...
	private static MappingsBuilder<T> AddContentTagsField<T>(this MappingsBuilder<T> m) where T : SearchDocumentBase =>
		m.ContentType(f => f.CopyTo("tags")).AddField("tags", f => f.Text().Analyzer(ContentTagsAnalyzer));
	...
	/// </summary>
	public static MappingsBuilder<T> AddSearchDocumentMappings<T>(
		this MappingsBuilder<T> m,
		bool semantic = false
