...
public static class SharedAnalysisFactory
{
	public static AnalysisBuilder BuildBaseAnalysis(AnalysisBuilder analysis) =>
		analysis.Normalizer(
			...
			.Analyzer("hierarchy_analyzer", a => a.Custom().Tokenizer("path_tokenizer"))
			.CharFilter("strip_non_word_chars", cf => cf.PatternReplace().Pattern(@"\W").Replacement(" "))
			.TokenFilter("english_stop", tf => tf.Stop().Stopwords("_english_"))
			.Tokenizer(
				"starts_with_tokenizer",
				t => t.EdgeNGram().MinGram(1).MaxGram(10).TokenChars("letter", "digit", "symbol", "whitespace")
			)
			.Tokenizer(
				"group_tokenizer",
				t => t.CharGroup().TokenizeOnChars("whitespace", ",", ";", "?", "!", "(", ")", "&", "'", "\"", "/", "[", "]", "{", "}")
			)
			.Tokenizer("path_tokenizer", t => t.PathHierarchy().Delimiter('/'))
			...
			.Analyzer(
				"content_tags_analyzer",
				a => a.Custom().Tokenizer("keyword").Filters("lowercase", "kstem", "content_tags_synonyms_filter")
			)
			.TokenFilter("content_tags_synonyms_filter", tf => tf.SynonymGraph().Synonyms("customer story, customer-story"));

	public static AnalysisBuilder BuildAnalysis(AnalysisBuilder analysis, string synonymSetName, string[] indexTimeSynonyms) =>
		BuildBaseAnalysis(analysis).Analyzer(
		...
		.CharFilter(
			"symbol_rewrite_char_filter",
			cf => cf.PatternReplace().Pattern(@"(?i)\bc#|(?<![a-zA-Z0-9])\.net\b").Replacement("dotnet")
		)
		...
		).TokenFilter("synonyms_fixed_filter", tf => tf.SynonymGraph().Synonyms(indexTimeSynonyms)).TokenFilter(
			"synonyms_filter",
			tf => tf.SynonymGraph().SynonymsSet(synonymSetName).Updateable(true)
		);
}
