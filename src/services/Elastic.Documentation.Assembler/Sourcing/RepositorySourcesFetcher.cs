...
public class AssemblerRepositorySourcer(ILoggerFactory logFactory, AssembleContext context)
{
	...
	public CheckoutResult GetAll()
	{
		...
		foreach (var repo in repositories.Values)
		{
			...
			IGitRepository gitFacade = new SingleCommitOptimizedGitRepository(
				logFactory,
				context.Collector,
				checkoutFolder,
				repo.CloneTimeout
...
)
{
	...
	)
	{
		...
		else
		{
			...
			{
				_logger.LogError(
					"{RepositoryName}: Fetch failed for {GitRef}, falling back to recreating from scratch",
					repository.Name,
					gitRef
				);
				checkoutFolder.Delete(true);
			...
			{
				_logger.LogError(
					"{RepositoryName}: Pull failed for {GitRef} from {Path}, falling back to recreating from scratch",
					repository.Name,
					gitRef,
					checkoutFolder.FullName
				);
