...
public record AssemblerDocumentationSet
{
	...
	)
	{
		...
		)
		{
			...
			GoogleTagManagerEnabled = env.GoogleTagManager.Enabled,
			GoogleTagManagerScript = env.GoogleTagManager.Script,
			GoogleTagManagerNoScript = env.GoogleTagManager.NoScript,
