...
public abstract record AskAiEvent(string Id, long Timestamp)
{
	...
	/// </summary>
	public sealed record ConversationStart(string Id, long Timestamp, string ConversationId) : AskAiEvent(Id, Timestamp);

	/// <summary>
	/// Streaming text chunk from AI
	/// </summary>
	public sealed record MessageChunk(string Id, long Timestamp, string Content) : AskAiEvent(Id, Timestamp);

	/// <summary>
	/// Complete message when streaming is done
	/// </summary>
	public sealed record MessageComplete(string Id, long Timestamp, string FullContent) : AskAiEvent(Id, Timestamp);

	/// <summary>
	/// AI is calling the search tool with a specific query
	/// </summary>
	public sealed record SearchToolCall(string Id, long Timestamp, string ToolCallId, string SearchQuery) : AskAiEvent(Id, Timestamp);

	/// <summary>
	/// AI is calling a tool (generic fallback for unknown tools)
	/// </summary>
	public sealed record ToolCall(string Id, long Timestamp, string ToolCallId, string ToolName, string Arguments) : AskAiEvent(
		Id,
		Timestamp
	);
	...
	/// </summary>
	public sealed record ToolResult(string Id, long Timestamp, string ToolCallId, string Result) : AskAiEvent(Id, Timestamp);

	/// <summary>
	/// AI is reasoning/thinking (e.g., searching, planning)
	/// </summary>
	public sealed record Reasoning(string Id, long Timestamp, string? Message) : AskAiEvent(Id, Timestamp);

	/// <summary>
	/// Conversation has ended
	/// </summary>
	public sealed record ConversationEnd(string Id, long Timestamp) : AskAiEvent(Id, Timestamp);

	/// <summary>
	/// An error occurred
	/// </summary>
	public sealed record ErrorEvent(string Id, long Timestamp, string Message) : AskAiEvent(Id, Timestamp);
}
