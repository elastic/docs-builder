...
public class LlmGatewayStreamTransformer(ILogger<LlmGatewayStreamTransformer> logger) : StreamTransformerBase(logger)
{
	...
	protected override AskAiEvent? TransformJsonEvent(string? eventType, JsonElement json)
	{
		...
		return type switch
		{
			...
			"tool_call" when messageData.TryGetProperty("toolCalls", out var toolCalls) => TransformToolCall(id, timestamp, toolCalls),
			...
			"agent_end" => new AskAiEvent.ConversationEnd(id, timestamp),
			"error" => ParseErrorEvent(id, timestamp, messageData),
			"chat_model_start" or "chat_model_end" => null, // Skip model lifecycle events
	...
	private AskAiEvent? TransformToolCall(string id, long timestamp, JsonElement toolCalls)
	{
		try
		{
			...
			return new AskAiEvent.ToolCall(id, timestamp, toolCallId ?? id, toolName ?? "unknown", args.GetRawText());
		}
		catch (Exception ex)
		{
	...
	private AskAiEvent.ErrorEvent ParseErrorEvent(string id, long timestamp, JsonElement messageData)
	{
		...
		var errorMessage = messageData.TryGetProperty("message", out var msgProp)
			? msgProp.GetString()
			: messageData.TryGetProperty("error", out var errProp) ? errProp.GetString() : null;
	...
	private AskAiEvent.ErrorEvent ParseAgentStreamError(JsonElement json)
	{
		...
		var errorMessage = json.TryGetProperty("error", out var errorProp) ? errorProp.GetString() : null;
