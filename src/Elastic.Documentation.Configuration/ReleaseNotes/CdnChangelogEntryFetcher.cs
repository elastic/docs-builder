...
public sealed class CdnChangelogEntryFetcher : IDisposable
{
	...
	)
	{
		...
		try
		{
			...
			{
				_logger.LogDebug(
					"Notes index for {Org}/{Repo}@{Target} not found at {Uri}; no notes to bundle",
					org,
					repo,
					target,
					indexUri
		...
		foreach (var poolRelativePath in index.Notes)
		{
			...
			{
				emitError(
					$"Notes index for {repoLabel}@{target} lists an invalid pool-relative path '{poolRelativePath}'; expected {{branch}}/{{file}}."
				);
				return [];
			}
