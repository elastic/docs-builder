...
{
	VersioningSystem InferVersion(
		string repositoryName,
		IReadOnlyCollection<LegacyPageMapping>? legacyPages,
		IReadOnlyCollection<Product>? products,
		ApplicableTo? applicableTo
	);
}

public class ProductVersionInferrerService(
...
) : IVersionInferrerService
{
	...
	public VersioningSystem InferVersion(
		string repositoryName,
		IReadOnlyCollection<LegacyPageMapping>? legacyPages,
		IReadOnlyCollection<Product>? products,
		ApplicableTo? applicableTo
	)
	{
		...
		{
			var versioningFromApplicability = VersioningFromApplicability(
				applicableTo
			); // Try to infer the versioning system from the applicability metadata
...
public class NoopVersionInferrer : IVersionInferrerService
{
	public VersioningSystem InferVersion(
		...
		IReadOnlyCollection<Product>? products,
		ApplicableTo? applicableTo
	) => new() { Id = VersioningSystemId.Stack, Base = ZeroVersion.Instance, Current = ZeroVersion.Instance };
}
