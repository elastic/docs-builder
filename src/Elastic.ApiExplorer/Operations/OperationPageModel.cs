...
public record ExampleScenario
{
	...
	public bool ShowRequest =>
		(RequestJson is not null || !string.IsNullOrEmpty(RequestExternalValue)) && CodeSamples.Count == 0;
...
public partial record OperationPageModel
{
		...
		IReadOnlyList<CodeSample> codeSamples)
	{
		...
		if (hasRequestScenarios && sharedResponses.Count > 0)
		{
			for (var i = 0; i < scenarios.Count; i++)
				scenarios[i] = scenarios[i] with
				{
					Responses = MergeSharedResponses(scenarios[i].Responses, sharedResponses)
				};
		}
		...
		{
			scenarios.Add(new ExampleScenario
			{
				Title = "Examples",
				TabId = "examples",
	...
	private static List<ExampleScenario> CollapseIntoSingleScenario(List<ExampleScenario> scenarios)
	{
		...
		foreach (var scenario in scenarios)
		{
			...
			{
				var alreadyPresent = responses.Any(r =>
					string.Equals(r.StatusCode, response.StatusCode, StringComparison.OrdinalIgnoreCase));
				if (alreadyPresent)
					continue;
		...
		var ordered = responses
			.OrderBy(r => StatusSortKey(r.StatusCode))
	...
	private static IReadOnlyList<ExampleResponse> UpsertResponse(
		IReadOnlyList<ExampleResponse> existing,
		ExampleDisplay example)
	{
		var statusCode = string.IsNullOrEmpty(example.StatusCode) ? "default" : example.StatusCode;
		var next = new ExampleResponse
		{
			StatusCode = statusCode,
			JsonValue = example.JsonValue,
			ExternalValue = example.ExternalValue
		};
		var list = existing.ToList();
		var index = list.FindIndex(r =>
			string.Equals(r.StatusCode, statusCode, StringComparison.OrdinalIgnoreCase));
		if (index >= 0)
			list[index] = next;
		else
			list.Add(next);
	...
	private static int StatusSortKey(string statusCode) =>
		statusCode.Length > 0 && statusCode[0] == '2' ? 0
		: statusCode.Length > 0 && statusCode[0] == '3' ? 1
		: statusCode.Length > 0 && statusCode[0] == '4' ? 2
		: statusCode.Length > 0 && statusCode[0] == '5' ? 3
		: 4;

	private static int FindScenarioForCodeSamples(
		IReadOnlyList<ExampleScenario> scenarios,
		IReadOnlyList<CodeSample> codeSamples)
	{
		var probe = codeSamples.FirstOrDefault(static s =>
				string.Equals(s.Language, "Console", StringComparison.OrdinalIgnoreCase))
			?? codeSamples[0];
