...
function linkPathMatchesCurrentPage(anchor: HTMLAnchorElement) {
	...
	const linkPath = normalizeDocPathname(new URL(href, window.location.href).pathname)
...
function ensureNavV2FolderLinkToggle() {
	...
	document.addEventListener(
		...
		(e: MouseEvent) => {
			...
			const cb = peer?.querySelector<HTMLInputElement>(':scope > input[type="checkbox"]')
	...
	document.addEventListener(
		...
		(e: MouseEvent) => {
			...
			if (
				...
				e.target.closest('nav[data-nav-v2] .docs-sidebar-nav-v2__label--top')
			) {
				return
			}

			const target = e.target
			nav.querySelectorAll<HTMLInputElement>('li.group-navigation input[type="checkbox"]:checked').forEach((cb) => {
...
function clearActiveSubtreeHighlight(nav: HTMLElement) {
	nav.querySelectorAll('.nav-v2-active-subtree, .nav-v2-active-leaf').forEach((el) => {
...
function applyActiveSubtreeHighlight(nav: HTMLElement) {
	...
	const current = nav.querySelector<HTMLAnchorElement>('a.sidebar-link.current')
...
function expandToCurrentPage(nav: HTMLElement) {
	...
	while (el && el !== nav) {
		if (el.matches('li')) {
			const cb = el.querySelector<HTMLInputElement>(':scope > .peer input[type=checkbox]')
			if (cb && cb.id) {
				const rowLink = el.querySelector<HTMLElement>(
					':scope > .nav-folder-peer > a.sidebar-link'
				)
				const currentIsThisFolderRow = rowLink !== null && rowLink === link
