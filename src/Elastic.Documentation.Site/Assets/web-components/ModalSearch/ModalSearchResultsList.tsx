...
import { SanitizedHtmlContent } from '../NavigationSearch/SanitizedHtmlContent'
...
import { useModalSearchQuery, SearchResultItem } from './useModalSearchQuery'
...
export interface ModalSearchResultsListProps {
    isKeyboardNavigating: MutableRefObject<boolean>
    onMouseMove: () => void
    onResultClick: () => void
}

export const ModalSearchResultsList = ({
    isKeyboardNavigating,
    onMouseMove,
    onResultClick,
}: ModalSearchResultsListProps) => {
    const { euiTheme } = useEuiTheme()
    const selectedIndex = useSelectedIndex()
    const { setSelectedIndex } = useModalSearchActions()
    const { isLoading, data } = useModalSearchQuery()
    const containerRef = useRef<HTMLDivElement>(null)
    const searchTerm = useSearchTerm()
    const { trackResultClicked } = useModalSearchTelemetry()
    ...
    useEffect(() => {
        if (selectedIndex < 0 || !isKeyboardNavigating.current) return
        ...
        if (elRect.bottom > ctRect.bottom) {
            container.scrollTop += elRect.bottom - ctRect.bottom
        } else if (elRect.top < ctRect.top) {
            container.scrollTop -= ctRect.top - elRect.top
        }
    }, [selectedIndex])
...
interface ModalSearchResultRowProps {
    index: number
