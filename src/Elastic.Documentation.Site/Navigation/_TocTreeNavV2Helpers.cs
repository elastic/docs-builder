...
internal static class TocTreeNavV2Helpers
{
	internal static string NavLinkClass(string? activeNavigationUrl, string url) =>
		NavigationActiveUrlResolver.IsActive(activeNavigationUrl, url)
			? "sidebar-link nav-v2-link current flex w-full min-w-0 items-center gap-[6px] rounded-[6px] py-[6px]"
			: "sidebar-link nav-v2-link flex w-full min-w-0 items-center gap-[6px] rounded-[6px] py-[6px]";
