...
public class SearchIntegrationTests(SearchBootstrapFixture searchFixture, ITestOutputHelper output) : SearchTestBase
{
	...
	public async Task SearchEndpointReturnsExpectedFirstResult(string query, string expectedFirstResultUrl)
	{
		...
		var searchResponse = await response.Content.ReadFromJsonAsync<FindPageApiResponse>(cancellationToken: TestContext.Current.CancellationToken);
	...
	public async Task SearchEndpointWithPaginationReturnsCorrectPage()
	{
		...
		var page1Data = await page1Response.Content.ReadFromJsonAsync<FindPageApiResponse>(cancellationToken: TestContext.Current.CancellationToken);
		...
		var page2Data = await page2Response.Content.ReadFromJsonAsync<FindPageApiResponse>(cancellationToken: TestContext.Current.CancellationToken);
