...
- **C#**: TUnit · AwesomeAssertions · FakeItEasy across all test projects. All unit test projects use TUnit (`<UseTUnit>true</UseTUnit>` in their csproj). Write new tests using TUnit APIs: `[Test]`, `[Arguments(...)]`, `[MethodDataSource(nameof(X))]`. Integration tests (`tests-integration/`) still use xUnit v3.
- **TypeScript**: Jest
