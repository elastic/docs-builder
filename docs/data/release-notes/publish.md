...
1. [Create a changelog configuration file](/data/release-notes/configure.md) to define all the default behavior and optional profiles and rules.
1. [Create changelogs](/data/release-notes/create.md) that describe all the notable changes.
1. [Bundle the changelogs](/data/release-notes/bundle.md) for product releases.
...
:::{important}
Before adding this directive, register the product in `_docset.yml`:
...
:::{note}
The CDN registry for a product is created by the first published bundle. If no release has gone through the bundle pipeline yet, the build fails with a 404. Add the render page only after the first release has run successfully.
:::
