...
For the most up-to-date changelog configuration options, refer to [changelog.example.yml](https://github.com/elastic/docs-builder/blob/main/config/changelog.example.yml) and [](/data/release-notes/configure-ref.md).
...
However it's a good idea to define the [basic bundle settings](/data/release-notes/configure-ref.md#bundle-basic) and the [profile settings](/data/release-notes/configure-ref.md#bundle-profiles) for the output filename and output products.
For example:
...
`--remove` needs a changelog file on disk so it can match name and checksum. Inferred git-ref entries have no such file; use `--force` with a dummy path named `{pull-request-number}.yaml`. Refer to [](/cli/changelog/cmd-bundle-amend.md#inferred-git-ref-entry).
...
For details, refer to [](/data/release-notes/configure-ref.md#rules-bundle) and [](/data/release-notes/configure-ref.md#advanced-rule-examples).
...
If there are features or projects that are not yet ready for public documentation, you can list those IDs in the [`hide_features`](/data/release-notes/configure-ref.md#bundle-profiles) setting:
