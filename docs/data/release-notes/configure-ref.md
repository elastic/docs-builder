...
| `bundle.owner`            | GitHub repository owner (default: `elastic`). **Keep this setting if your repository owner is not `elastic`.** It is used as the org segment of uploaded changelog-entry keys (`changelog/{org}/{repo}/{branch}/...`) and CDN entry sourcing. Release-based flows (`--release-version`, `source: github_release`) resolve owner as `--owner` > `bundle.owner` > `"elastic"` with no automatic env or git derivation. |
| `bundle.release_dates`    | When `true`, bundles include a `release-date` field (default: true). |
| `bundle.repo`             | GitHub repository name (for example, `elasticsearch`). Derived from `GITHUB_REPOSITORY` or the git remote `origin` for standard bundle flows — remove this setting if the derived value is correct. **Required for `--release-version` and `source: github_release` flows** unless you always pass `--repo` on the command line. Also the `{repo}` segment of `{repo}-{product}-{version}.yaml` names and of uploaded changelog-entry keys (`changelog/{org}/{repo}/{branch}/...`). |
