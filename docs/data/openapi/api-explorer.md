...
Additionally, the slug must not match any operation segment already generated by the spec. The
...
When a local spec file exists, it overrides only the `main` key. Older majors still resolve
remotely through the index.

Versionless products (`versioning: serverless` and similar) render only the unversioned
`/api/doc/<key>/` path even when the index lists historical keys. When more than one
