...
The assembler also writes a combined **API catalog** at `/docs/api/`: a grid of product cards on its own layout (no API sidebar). Each card opens the HTML landing page. Markdown and JSON links on the card open the landing-page export and the OpenAPI spec. The card teaser is the first paragraph of the spec `info.description`.
