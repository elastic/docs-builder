...
`--output <string?>`
:   Optional: Output directory for the generated changelog files. Falls back to `bundle.directory` in `changelog.yml` or if that's not set then `./changelogs`.
...
This command also performs private link sanitization if `bundle.sanitize_private_links` and `bundle.resolve` are set to true in the changelog configuration file.
For behavior and registry requirements, refer to [Private link sanitization at bundle time](/contribute/changelog.md#changelog-bundle-private-link-sanitization).
