...
`--issues <string[]?>`
:   Optional: Issue URLs or numbers (comma-separated or specify multiple times), or a path to a newline-delimited file containing issue URLs or numbers.
...
:    Required if `--prs` is not specified.
