...
1. Set up the new docs folder structure and CI configuration.
1. Configure the repository source in `assembler.yml`. See [](./site/content.md) for details.

  :::{note}
  The tagged branching strategy requires first setting up the new docs folder structure and CI integration, then configuring the documentation CI integration.
...
Also add **`docs-deploy.yml`** and **`docs-preview-cleanup.yml`** using the reusable workflows in [`elastic/docs-actions`](https://github.com/elastic/docs-actions); the snippet below only highlights the tagged-branching-specific parts of **docs-build** (and the same `with:` block must be passed through on your **docs-deploy** consumer job).
