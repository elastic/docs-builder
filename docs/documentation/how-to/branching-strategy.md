...
For more information, refer to [content sources](/documentation/configure/content-sources.md).

After it has been established that a repository should publish from a version branch rather than `main`:

1. [Add new triggers to the `docs-build` CI integration](/documentation/configure/content-sources.md#ci-configuration). Merge these changes to `main` or `master` and the intended version branches.
